              ...
              <tr>
                  ...
                  <td>
                      ...
                      <a class="btn btn-sm btn-info" href="{{ route('timesheets.tasks.show', [$timesheet->id, $task->id]) }}" >
                        {{ $task->name }} 
                        <form id="deleteTask" method="POST" action="{{ route('timesheets.tasks.destroy', [ $timesheet->id, $task->id]) }}" style="display: inline;" onsubmit="return confirm('Delete? Are you sure?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="close" data-dismiss="modal" aria-label="Close" style="outline: none">
                              <span>&times;</span>
                          </button>
                        </form>
                      </a>
                      @endforeach
                      <a class="btn btn-success float-center" href="{{ route('timesheets.tasks.create', $timesheet->id) }}" data-toggle="modal" data-target="#createTask"><i class="fas fa-plus"></i></a>
                  </td>
